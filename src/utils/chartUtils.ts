...
export const EDGE_LEFT = 0.20;
export const EDGE_RIGHT = 0.75;

export const SLOT_COUNT: Record<sensorLogScale, number> = {
  minute: 120, hour: 48, day: 14, week: 8, month: 24, year: 8,
};

export const SPACING_PER_SCALE: Record<sensorLogScale, number> = {
  minute: 60, hour: 80, day: 100, week: 120, month: 140, year: 180,
};

export const sensorOption: sensorLogScale[] = ['minute', 'hour', 'day', 'week', 'month', 'year'];
...
export function addStep(d: Date, scale: sensorLogScale, n: number): Date {
  const x = new Date(d);
  if (scale === 'minute') x.setMinutes(x.getMinutes() + n);
