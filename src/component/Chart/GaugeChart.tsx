...
const HalfCircleGauge: React.FC<Props> = ({
  ...
  if(threshold_max !== undefined && threshold_min !== undefined){
    if(value <= threshold_min || value >= threshold_max ){
      color = "#F77979" // red
    }
    if(value < threshold_min * 0.5 || value > threshold_max * 0.5){
      color = "#F2BC79" // orange
    } 
    if(value > threshold_min && value < threshold_max)
    {
  ...
  return (
    <View style={{ width: size, alignItems: "center" }}>
      ...
      {/* Gauge */}
      {loading ? <LoadingSpinner size='small'/> : <View style={{ width: size, height: size / 2, overflow: "hidden" }}>
